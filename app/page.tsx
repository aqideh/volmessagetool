...
export default function Home() {
  ...
  function multiShiftVolunteerIds(): string[] {
    ...
    eventAssignments.forEach((assignment) => counts.set(assignment.volunteerId, (counts.get(assignment.volunteerId) || 0) + 1));
    return eventVolunteers.filter((volunteer) => (counts.get(volunteer.id) || 0) >= 2).map((volunteer) => volunteer.id);
  }
  ...
  function audienceLabel(campaign: CampaignRecord) {
    if (campaign.audienceType === "shift") return eventShifts.find((shift) => shift.id === campaign.shiftId)?.name || "Missing target shift";
  ...
  function incompleteSummaryShifts(volunteerIds: string[]): string[] {
    ...
    const shiftIds = new Set(eventAssignments.filter((assignment) => targetIds.has(assignment.volunteerId)).map((assignment) => assignment.shiftId));
  ...
  async function saveCampaign() {
    ...
    if (usesShiftSummary) {
      const ids = audienceType === "multi_shift" ? multiShiftVolunteerIds() : audienceType === "shift" && campaignShiftId
        ? [...new Set(eventAssignments.filter((assignment) => assignment.shiftId === campaignShiftId).map((assignment) => assignment.volunteerId))]
  ...
  const previewVolunteer = audienceType === "multi_shift"
    ? eventVolunteers.find((volunteer) => assignmentsFor(volunteer.id).length >= 2)
    : eventVolunteers[0];
