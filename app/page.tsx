...
import { assignmentsForVolunteer, multiShiftVolunteerIds as getMultiShiftVolunteerIds } from "@/lib/assignments";
import { formatDisplayDate } from "@/lib/date";
import { db } from "@/lib/db";
import { parseRoster } from "@/lib/import";
import { DEFAULT_TEMPLATE, renderMessage, shiftVariablesUsed, templateVariables } from "@/lib/message";
...
export default function Home() {
  ...
  function assignmentFor(volunteerId: string, shiftId?: string) {
    ...
    return assignmentsFor(volunteerId).find((assignment) => assignment.shiftId === shiftId);
  }

  function multiShiftVolunteerIds(): string[] {
    const validVolunteerIds = new Set(eventVolunteers.map((volunteer) => volunteer.id));
    return getMultiShiftVolunteerIds(eventAssignments, eventShifts).filter((volunteerId) => validVolunteerIds.has(volunteerId));
  }

  function singleShiftVolunteerIdsForShift(shiftId: string): string[] {
    const multiShiftIds = new Set(multiShiftVolunteerIds());
    return [...new Set(
      eventAssignments
        .filter((assignment) => assignment.shiftId === shiftId && !multiShiftIds.has(assignment.volunteerId))
  ...
  function eligibleVolunteerIds(campaign: CampaignRecord | undefined): string[] {
    if (!campaign) return [];
    // Closed campaigns are historical snapshots; reopening recalculates eligibility.
    ...
    if (campaign.audienceType === "shift" && campaign.shiftId) {
      return singleShiftVolunteerIdsForShift(campaign.shiftId);
    }
    if (campaign.audienceType === "multi_shift") return multiShiftVolunteerIds();
    return eventVolunteers.map((volunteer) => volunteer.id);
  }

  function audienceLabel(campaign: CampaignRecord) {
    if (campaign.audienceType === "shift") return `${eventShifts.find((shift) => shift.id === campaign.shiftId)?.name || "Missing target shift"} · single-shift only`;
    if (campaign.audienceType === "multi_shift") return "Multiple-shift volunteers";
    return "All volunteers";
  }

  function incompleteSummaryShifts(volunteerIds: string[]): string[] {
    const targetIds = new Set(volunteerIds);
    const shiftIds = new Set(
      eventVolunteers
        .filter((volunteer) => targetIds.has(volunteer.id))
        .flatMap((volunteer) => assignmentsFor(volunteer.id).map((assignment) => assignment.shiftId)),
    );
    const problems: string[] = [];
    shiftIds.forEach((shiftId) => {
      const shift = eventShifts.find((item) => item.id === shiftId);
      if (!shift) {
        problems.push("Missing shift record");
        return;
      }
      const missing = missingShiftFields(shift);
      if (missing.length) problems.push(`${shift.name || "Unnamed shift"} (${missing.join(", ")})`);
    });
    return problems;
  }

  const campaignVolunteerIds = useMemo(() => eligibleVolunteerIds(selectedCampaign), [selectedCampaign, eventAssignments, eventVolunteers, eventShifts]);
  ...
  const filteredVolunteers = eventVolunteers.filter((volunteer) => {
    ...
    const volunteerAssignments = assignmentsFor(volunteer.id);
    const matchesShift = shiftFilter === "all" || volunteerAssignments.some((assignment) => assignment.shiftId === shiftFilter);
    const shiftNames = volunteerAssignments.map((assignment) => eventShifts.find((shift) => shift.id === assignment.shiftId)?.name || "").join(" ");
    return matchesShift && (!q || `${volunteer.name} ${volunteer.phone} ${shiftNames} ${volunteerAssignments.map((item) => item.role).join(" ")}`.toLowerCase().includes(q));
  });
  ...
  function beginAssignmentEdit(volunteer: VolunteerRecord) {
    const drafts = assignmentsFor(volunteer.id).map((assignment) => ({ id: assignment.id, shiftId: assignment.shiftId, role: assignment.role }));
  ...
  async function saveCampaign() {
    ...
    if (audienceType !== "shift" && usedShiftVariables.length) return setNotice(`This audience cannot use single-shift variables: ${usedShiftVariables.map((variable) => `{{${variable}}}`).join(", ")}. Use {{shift_summary}} instead.`);
    ...
    const usesShiftSummary = templateVariables(template).includes("shift_summary");
    if (usesShiftSummary) {
      const ids = audienceType === "multi_shift"
        ? multiShiftVolunteerIds()
        : audienceType === "shift" && campaignShiftId
          ? singleShiftVolunteerIdsForShift(campaignShiftId)
          : eventVolunteers.map((volunteer) => volunteer.id);
      const incomplete = incompleteSummaryShifts(ids);
      if (incomplete.length) return setNotice(`Complete all shifts used in {{shift_summary}} before saving: ${incomplete.join("; ")}.`);
    }
  ...
  async function toggleCampaignClosed(campaign: CampaignRecord) {
    if (campaign.status === "active") {
      ...
      setNotice("Campaign reopened. Eligibility was recalculated using the current shift assignments.");
    }
  ...
  async function openWhatsApp(volunteer: VolunteerRecord) {
    ...
    if (templateVariables(selectedCampaign.template).includes("shift_summary")) {
      const incomplete = incompleteSummaryShifts([volunteer.id]);
      if (incomplete.length) return setNotice(`Cannot generate ${volunteer.name}'s shift summary: ${incomplete.join("; ")}.`);
    }
    const assignment = shift ? assignmentFor(volunteer.id, shift.id) : undefined;
    const message = renderMessage(selectedCampaign.template, selectedEvent, volunteer, shift, assignment, eventShifts, eventAssignments);
  ...
  const previewVolunteer = audienceType === "multi_shift"
    ? eventVolunteers.find((volunteer) => multiShiftVolunteerIds().includes(volunteer.id))
    : audienceType === "shift" && campaignShiftId
      ? eventVolunteers.find((volunteer) => singleShiftVolunteerIdsForShift(campaignShiftId).includes(volunteer.id))
      : eventVolunteers[0];
  const previewAssignment = previewVolunteer && previewShift ? assignmentFor(previewVolunteer.id, previewShift.id) : undefined;
  const currentShiftVariableConflict = audienceType !== "shift" ? shiftVariablesUsed(template) : [];
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {!selectedEvent ? <div className="empty-state"><h1>Start with an event</h1><p>Create an event, then explicitly configure its shifts before adding volunteers.</p></div> : <>
          <header className="event-header"><div><p className="eyebrow">{selectedEvent.status}</p><h1>{selectedEvent.name}</h1><p>{formatDisplayDate(selectedEvent.date)}{selectedEvent.time ? ` - ${selectedEvent.time}` : ""}{selectedEvent.venue ? ` - ${selectedEvent.venue}` : ""}</p></div><button className="secondary" onClick={archiveEvent}>{selectedEvent.status === "active" ? "Archive event" : "Restore event"}</button></header>
