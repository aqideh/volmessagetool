...
const emptyShiftForm = (event?: EventRecord): ShiftForm => ({
  name: "",
  date: event?.date || "",
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setShiftForm((current) => ({
      ...current,
      date: current.date || selectedEvent.date,
      startTime: current.startTime || selectedEvent.time,
      reportingTime: current.reportingTime || selectedEvent.time,
      venue: current.venue || selectedEvent.venue,
    }));
  }, [selectedEvent?.id]);

  useEffect(() => {
    const firstShift = eventShifts[0]?.id || "";
    if (!newVolunteer.shiftId || !eventShifts.some((shift) => shift.id === newVolunteer.shiftId)) {
      setNewVolunteer((current) => ({ ...current, shiftId: firstShift }));
    }
    if (!importShiftId || !eventShifts.some((shift) => shift.id === importShiftId)) setImportShiftId(firstShift);
    if (!newAssignment.shiftId || !eventShifts.some((shift) => shift.id === newAssignment.shiftId)) {
      setNewAssignment((current) => ({ ...current, shiftId: firstShift }));
    }
  ...
  const campaignVolunteers = campaignVolunteerIds
    .map((volunteerId) => eventVolunteers.find((volunteer) => volunteer.id === volunteerId))
    .filter(Boolean) as VolunteerRecord[];
  ...
  const counts = campaignVolunteers.reduce(
    (acc, volunteer) => {
  ...
  async function createShift(event: FormEvent) {
    ...
    setShiftForm(emptyShiftForm(selectedEvent));
  ...
  function beginShiftEdit(shift: ShiftRecord) {
    ...
    setShiftEditForm({
      ...
      reportingTime: shift.reportingTime,
      venue: shift.venue,
      notes: shift.notes,
    });
  }
  ...
  async function saveShiftEdit(shiftId: string) {
    if (!shiftEditForm.name.trim() || !shiftEditForm.date) return setNotice("Shift name and date are required.");
    const duplicate = eventShifts.some(
      (shift) => shift.id !== shiftId && normalizeShiftName(shift.name) === normalizeShiftName(shiftEditForm.name),
    );
    if (duplicate) return setNotice("A shift with that name already exists for this event.");
    await db.shifts.update(shiftId, { ...shiftEditForm, name: shiftEditForm.name.trim() });
    setEditingShiftId("");
    setNotice("Shift details updated. Existing campaign links to this shift were preserved.");
    ...
    try {
      ...
      await db.transaction("rw", db.volunteers, db.assignments, async () => {
        for (const row of rows) {
          ...
          if (!row.name || !phone || !targetShiftId) {
            skipped += 1;
            continue;
          }
  ...
  function beginAssignmentEdit(volunteer: VolunteerRecord) {
    const drafts = eventAssignments
      .filter((assignment) => assignment.volunteerId === volunteer.id)
      .map((assignment) => ({ id: assignment.id, shiftId: assignment.shiftId, role: assignment.role }));
    const used = new Set(drafts.map((draft) => draft.shiftId));
    const firstAvailable = eventShifts.find((shift) => !used.has(shift.id))?.id || eventShifts[0]?.id || "";
    setEditingVolunteerId(volunteer.id);
    setAssignmentDrafts(drafts);
    setNewAssignment({ shiftId: firstAvailable, role: "" });
  }
  ...
  async function saveAssignmentChanges(volunteer: VolunteerRecord) {
    ...
    if (shiftIds.length !== assignmentDrafts.length) return setNotice("Every assignment needs a shift.");
    ...
    await db.transaction("rw", db.assignments, async () => {
      for (const draft of assignmentDrafts) {
        await db.assignments.update(draft.id, { shiftId: draft.shiftId, role: draft.role.trim() });
      }
    });
  ...
  async function addAssignmentToVolunteer(volunteer: VolunteerRecord) {
    if (!selectedEvent || !newAssignment.shiftId) return setNotice("Choose a shift to add.");
    const currentShiftIds = new Set(assignmentDrafts.map((draft) => draft.shiftId));
    if (currentShiftIds.has(newAssignment.shiftId)) return setNotice("That volunteer is already assigned to this shift.");
    const record: AssignmentRecord = {
      id: id(),
      eventId: selectedEvent.id,
      shiftId: newAssignment.shiftId,
      volunteerId: volunteer.id,
      role: newAssignment.role.trim(),
      createdAt: now(),
    };
    await db.assignments.add(record);
    const nextDrafts = [...assignmentDrafts, { id: record.id, shiftId: record.shiftId, role: record.role }];
    setAssignmentDrafts(nextDrafts);
    const used = new Set(nextDrafts.map((draft) => draft.shiftId));
    const nextAvailable = eventShifts.find((shift) => !used.has(shift.id))?.id || "";
    setNewAssignment({ shiftId: nextAvailable, role: "" });
  ...
  async function saveCampaign() {
    ...
    if (selectedCampaign) {
      ...
      await db.campaigns.update(selectedCampaign.id, {
        name: campaignName.trim() || "Untitled message",
        template,
        audienceType,
        shiftId: audienceType === "shift" ? campaignShiftId : undefined,
        updatedAt: timestamp,
      });
  ...
  async function restoreBackup(file?: File) {
    ...
    try {
      ...
      if (raw.version === 2) {
        ...
        const legacyShifts = legacy.events.map((event) => ({
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="event-list">
          ...
          {events.map((event) => (
            <button key={event.id} className={`event-item ${event.id === selectedEventId ? "selected" : ""}`} onClick={() => { setSelectedEventId(event.id); setSelectedCampaignId(""); setEditingShiftId(""); setEditingVolunteerId(""); }}>
      ...
      <section className="workspace">
        ...
        {!selectedEvent ? (
          <div className="empty-state"><h1>Start with an event</h1><p>Create an event, then configure shifts, upload a roster and prepare WhatsApp messages.</p></div>
        ) : (
          <>
            ...
            <nav className="tabs">
              {(["shifts", "roster", "messages", "send", "backup"] as Tab[]).map((item) => (
                <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{item === "send" ? "Send queue" : item[0].toUpperCase() + item.slice(1)}</button>
              ))}
            </nav>
              ...
              <div className="stack">
                <section className="panel">
                  ...
                  <form className="shift-form" onSubmit={createShift}>
                    <input placeholder="Shift name" value={shiftForm.name} onChange={(event) => setShiftForm({ ...shiftForm, name: event.target.value })} />
                    <input type="date" value={shiftForm.date} onChange={(event) => setShiftForm({ ...shiftForm, date: event.target.value })} />
                    <input type="time" title="Start time" value={shiftForm.startTime} onChange={(event) => setShiftForm({ ...shiftForm, startTime: event.target.value })} />
                    <input type="time" title="End time" value={shiftForm.endTime} onChange={(event) => setShiftForm({ ...shiftForm, endTime: event.target.value })} />
                    <input type="time" title="Reporting time" value={shiftForm.reportingTime} onChange={(event) => setShiftForm({ ...shiftForm, reportingTime: event.target.value })} />
                    <input placeholder="Venue" value={shiftForm.venue} onChange={(event) => setShiftForm({ ...shiftForm, venue: event.target.value })} />
                    <input placeholder="Notes (optional)" value={shiftForm.notes} onChange={(event) => setShiftForm({ ...shiftForm, notes: event.target.value })} />
                    <button className="primary" type="submit">Add shift</button>
                  </form>
                </section>
                <section className="card-grid">
                  {eventShifts.map((shift) => {
                    const count = new Set(eventAssignments.filter((assignment) => assignment.shiftId === shift.id).map((assignment) => assignment.volunteerId)).size;
                    const isEditing = editingShiftId === shift.id;
                    return (
                      <article className={`panel shift-card ${isEditing ? "editing" : ""}`} key={shift.id}>
                        {isEditing ? (
                          <div className="shift-edit-form">
                            <label>Shift name<input value={shiftEditForm.name} onChange={(event) => setShiftEditForm({ ...shiftEditForm, name: event.target.value })} /></label>
                            <label>Date<input type="date" value={shiftEditForm.date} onChange={(event) => setShiftEditForm({ ...shiftEditForm, date: event.target.value })} /></label>
                            <label>Start time<input type="time" value={shiftEditForm.startTime} onChange={(event) => setShiftEditForm({ ...shiftEditForm, startTime: event.target.value })} /></label>
                            <label>End time<input type="time" value={shiftEditForm.endTime} onChange={(event) => setShiftEditForm({ ...shiftEditForm, endTime: event.target.value })} /></label>
                            <label>Reporting time<input type="time" value={shiftEditForm.reportingTime} onChange={(event) => setShiftEditForm({ ...shiftEditForm, reportingTime: event.target.value })} /></label>
                            <label>Venue<input value={shiftEditForm.venue} onChange={(event) => setShiftEditForm({ ...shiftEditForm, venue: event.target.value })} /></label>
                            <label className="wide-field">Notes<input value={shiftEditForm.notes} onChange={(event) => setShiftEditForm({ ...shiftEditForm, notes: event.target.value })} /></label>
                            <div className="actions wide-field"><button className="primary" onClick={() => saveShiftEdit(shift.id)}>Save changes</button><button className="secondary" onClick={() => setEditingShiftId("")}>Cancel</button></div>
                          </div>
                        ) : (
                          <>
                            <div><h2>{shift.name}</h2><p>{shift.date} - {shift.startTime || "Time not set"}{shift.endTime ? `-${shift.endTime}` : ""}</p><p className="muted">Report {shift.reportingTime || "-"} - {shift.venue || selectedEvent.venue || "Venue not set"}</p>{shift.notes && <p className="muted">{shift.notes}</p>}<strong>{count} volunteer{count === 1 ? "" : "s"}</strong></div>
                            <div className="shift-card-actions"><button className="secondary compact" onClick={() => beginShiftEdit(shift)}>Edit</button><button className="text-button danger" onClick={() => removeShift(shift)}>Remove</button></div>
                          </>
                        )}
                      </article>
                    );
            ...
            {tab === "roster" && (
              <div className="stack">
                <section className="panel split">
                  <div><h2>Roster</h2><p className="muted">Upload CSV/XLSX. If a Shift column is present it is matched by shift name; otherwise rows go to the fallback shift selected here.</p></div>
                  <div className="actions"><select value={importShiftId} onChange={(event) => setImportShiftId(event.target.value)}>{eventShifts.map((shift) => <option key={shift.id} value={shift.id}>Fallback: {shift.name}</option>)}</select><input ref={fileRef} className="file-input" type="file" accept=".csv,.xlsx,.xls" onChange={(event) => importRoster(event.target.files?.[0])} /><button className="secondary" onClick={() => fileRef.current?.click()}>Upload roster</button></div>
                </section>
                <section className="panel">
                  <form className="inline-form roster-add" onSubmit={addVolunteer}>
                    <input placeholder="Name" value={newVolunteer.name} onChange={(event) => setNewVolunteer({ ...newVolunteer, name: event.target.value })} />
                    <input placeholder="Phone" value={newVolunteer.phone} onChange={(event) => setNewVolunteer({ ...newVolunteer, phone: event.target.value })} />
                    <select value={newVolunteer.shiftId} onChange={(event) => setNewVolunteer({ ...newVolunteer, shiftId: event.target.value })}>{eventShifts.map((shift) => <option key={shift.id} value={shift.id}>{shift.name}</option>)}</select>
                    <input placeholder="Role for this shift" value={newVolunteer.role} onChange={(event) => setNewVolunteer({ ...newVolunteer, role: event.target.value })} />
                    <button className="primary" type="submit">Add / assign</button>
                  </form>
                </section>
                <section className="panel">
                  <div className="table-toolbar"><strong>{eventVolunteers.length} volunteer{eventVolunteers.length === 1 ? "" : "s"}</strong><div className="actions"><select value={shiftFilter} onChange={(event) => setShiftFilter(event.target.value)}><option value="all">All shifts</option>{eventShifts.map((shift) => <option key={shift.id} value={shift.id}>{shift.name}</option>)}</select><input className="search" placeholder="Search roster" value={search} onChange={(event) => setSearch(event.target.value)} /></div></div>
                  <div className="table-wrap">
                    <table>
                      <thead><tr><th>Name</th><th>Phone</th><th>Shift assignments</th><th></th></tr></thead>
                      <tbody>
                          ...
                          return (
                            <Fragment key={volunteer.id}>
                              <tr>
                                ...
                                <td><div className="assignment-list">{volunteerAssignments.length === 0 && <span className="muted">No shift assigned</span>}{volunteerAssignments.map((assignment) => { const shift = eventShifts.find((item) => item.id === assignment.shiftId); return <span className="assignment-chip" key={assignment.id}>{shift?.name || "Unknown shift"}{assignment.role ? ` - ${assignment.role}` : ""}</span>; })}</div></td>
                                <td><div className="row-actions"><button className="secondary compact" onClick={() => isEditing ? setEditingVolunteerId("") : beginAssignmentEdit(volunteer)}>{isEditing ? "Close" : "Manage shifts"}</button><button className="text-button danger" onClick={() => removeVolunteer(volunteer)}>Remove person</button></div></td>
                              </tr>
                              {isEditing && (
                                <tr className="assignment-manager-row">
                                  <td colSpan={4}>
                                    <div className="assignment-manager">
                                      <div className="assignment-manager-header"><div><strong>Manage {volunteer.name}'s assignments</strong><p className="muted">Change a shift to move the volunteer. Roles are specific to each shift.</p></div><button className="primary" onClick={() => saveAssignmentChanges(volunteer)}>Save assignments</button></div>
                                      {assignmentDrafts.length === 0 && <p className="muted">This volunteer is not assigned to any shift.</p>}
                                      <div className="assignment-editor-list">
                                        {assignmentDrafts.map((draft) => {
                                          const original = eventAssignments.find((assignment) => assignment.id === draft.id);
                                          return (
                                            <div className="assignment-editor-row" key={draft.id}>
                                              <select value={draft.shiftId} onChange={(event) => updateAssignmentDraft(draft.id, { shiftId: event.target.value })}>{eventShifts.map((shift) => <option key={shift.id} value={shift.id}>{shift.name}</option>)}</select>
                                              <input placeholder="Role (optional)" value={draft.role} onChange={(event) => updateAssignmentDraft(draft.id, { role: event.target.value })} />
                                              <button className="text-button danger" onClick={() => original && removeAssignment(original)}>Remove assignment</button>
                                            </div>
                                          );
                                        })}
                                      </div>
                                      {eventShifts.length > assignmentDrafts.length && (
                                        <div className="add-assignment-row"><select value={newAssignment.shiftId} onChange={(event) => setNewAssignment({ ...newAssignment, shiftId: event.target.value })}><option value="">Choose another shift</option>{eventShifts.filter((shift) => !assignmentDrafts.some((draft) => draft.shiftId === shift.id)).map((shift) => <option key={shift.id} value={shift.id}>{shift.name}</option>)}</select><input placeholder="Role for this shift" value={newAssignment.role} onChange={(event) => setNewAssignment({ ...newAssignment, role: event.target.value })} /><button className="secondary" onClick={() => addAssignmentToVolunteer(volunteer)}>+ Add assignment</button></div>
                                      )}
                                    </div>
                                  </td>
                                </tr>
            ...
            {tab === "messages" && (
              <div className="campaign-layout">
                <section className="panel campaign-list">
                  <div className="split"><div><h2>Messages</h2><p className="muted">Each message has its own audience and send history.</p></div><button className="primary" onClick={newCampaign}>+ New message</button></div>
                  {eventCampaigns.length === 0 && <p className="muted">No message campaigns yet.</p>}
                  {eventCampaigns.map((campaign) => <button className={`campaign-item ${selectedCampaignId === campaign.id ? "selected" : ""}`} key={campaign.id} onClick={() => setSelectedCampaignId(campaign.id)}><span><strong>{campaign.name}</strong><small>{campaign.audienceType === "shift" ? eventShifts.find((shift) => shift.id === campaign.shiftId)?.name || "Unknown shift" : "All volunteers"} - {campaign.status}</small></span><b>{campaignProgress(campaign)} sent</b></button>)}
                </section>
                <section className="panel message-editor">
                  <div className="split"><div><h2>{selectedCampaign ? "Edit message" : "New message"}</h2>{selectedCampaign && <p className="muted">Status: {selectedCampaign.status}</p>}</div>{selectedCampaign && <div className="actions"><button className="secondary" onClick={() => toggleCampaignClosed(selectedCampaign)}>{selectedCampaign.status === "active" ? "Close campaign" : "Reopen campaign"}</button><button className="text-button danger" onClick={() => deleteCampaign(selectedCampaign)}>Delete</button></div>}</div>
                  ...
                  <label>Audience<select disabled={selectedCampaign?.status === "closed"} value={audienceType} onChange={(event) => setAudienceType(event.target.value as CampaignAudienceType)}><option value="event">All volunteers in event</option><option value="shift">One shift</option></select></label>
                  {audienceType === "shift" && <label>Target shift<select disabled={selectedCampaign?.status === "closed"} value={campaignShiftId} onChange={(event) => setCampaignShiftId(event.target.value)}>{eventShifts.map((shift) => <option key={shift.id} value={shift.id}>{shift.name}</option>)}</select></label>}
                  <label>Template<textarea disabled={selectedCampaign?.status === "closed"} rows={13} value={template} onChange={(event) => setTemplate(event.target.value)} /></label>
                  <p className="muted">Variables: {"{{first_name}}"}, {"{{event_name}}"}, {"{{date}}"}, {"{{venue}}"}, {"{{role}}"}, {"{{shift_name}}"}, {"{{shift_date}}"}, {"{{shift_start}}"}, {"{{shift_end}}"}, {"{{reporting_time}}"}, {"{{shift_venue}}"}, plus spreadsheet fields.</p>
            ...
            {tab === "send" && (
              <div className="stack">
                ...
                {!selectedCampaign ? (
                  <section className="panel"><p className="muted">Create or select a message campaign first.</p><button className="primary" onClick={() => setTab("messages")}>Go to messages</button></section>
                ) : (
                  <>
